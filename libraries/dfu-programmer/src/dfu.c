...
retry:
    ...
    for (i = 0; i < devicecount; i++) {
        ...
        if ((vendor == descriptor.idVendor) &&
            ...
            ((bus_number == 0) || ((libusb_get_bus_number(device) == bus_number) &&
    ...
    if (0 < retries) {
        ...
        for (usb_bus = usb_get_busses(); NULL != usb_bus; usb_bus = usb_bus->next) {
            for (device = usb_bus->devices; NULL != device; device = device->next) {
                if ((vendor == device->descriptor.idVendor) && (product == device->descriptor.idProduct) && ((bus_number == 0) || (device->devnum == device_address && (usb_bus->location >> 24) == bus_number))) {
                    int32_t tmp;
    ...
    for (c = 0; c < device->descriptor.bNumConfigurations; c++) {
        ...
        for (i = 0; i < config->interface->num_altsetting; i++) {
            ...
            if (true == honor_interfaceclass) {
                /* Check if the interface is a DFU interface */
                if ((USB_CLASS_APP_SPECIFIC == interface->bInterfaceClass) && (DFU_SUBCLASS == interface->bInterfaceSubClass)) {
...
static void dfu_msg_response_output(const char *function,
    ...
    if (0 <= result) {
        ...
        switch (result) {
            ...
            case -EXDEV:
                msg =
                    "-EXDEV: ISO transfer only partially completed look at "
