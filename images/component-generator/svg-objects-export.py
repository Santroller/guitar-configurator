...
def exportObject(obj,args,prefix,extension,infile):
	debug("exporting ", obj)
	destfile = ''.join([args.destdir, prefix, obj, '.svg'])
	export = args.force
	if not args.force:
	...
	if export:
		...
		command = args.inkscape+' --export-area-page --export-id-only -i "'+obj+'" --export-'+args.type+' "'+destfile+'" '+args.extra+' "'+infile+'" '
		...
		if inkscape_v1_or_greater:
			if args.type == 'plain-svg':
				args.type = 'svg'
			command = args.inkscape+' --export-area-page --export-id-only -i "'+obj+'" --export-type='+args.type+' "'+destfile+'" '+args.extra+' "'+infile+'" '
