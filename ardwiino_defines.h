...
#include "submodules/Ardwiino/src/shared/input/input_ps2_cnt.h"
    ...
    static QString fixKey(QString string) {
          ...
          return sentence.join(" ").replace("Ps2","PS2").replace("Ps3","PS3").replace("Apa102", "APA102").replace("Midi","MIDI").replace("Dualshock","DualShock").replace("No","no").replace("Udraw","uDraw");
    }
