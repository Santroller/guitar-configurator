    ...
    if(ok)
    {
      ...
      if(cmdline.open(QIODevice::ReadOnly))
      {
        ...
        if(nullIdx > 0)
        {
          ...
          info.setCommand(firstparam);
          contents.replace('\0', ' ');
        } else {
           info.setCommand(QString::fromUtf8(contents).trimmed());
        }
...
const QString &QProcessInfo::command() const
{
  return m_cmd;
}

void QProcessInfo::setCommand(const QString &cmd)
{
  m_cmd = cmd;
}
